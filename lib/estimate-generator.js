...
class EstimateGenerator {
  constructor(templatePath = null) {
    // Hardcode the project root for Next.js API routes to avoid path resolution issues
     const projectRoot = 'D:\\Code\\Job\\Chatbot PDF\\nextjs';
     
     this.templatePath = templatePath || path.join(projectRoot, 'public', 'Sample Estimate Output (1).html');
     this.outputDir = path.join(projectRoot, 'htmls-date');
  ...
  generateEstimate(data, filename = null) {
    try {
      ...
      let html = template
        ...
        .replace(/1\/30\/23/g, data.estimateDate || '1/30/23')
      ...
      if (data.clientInfo) {
        html = html.replace(/<h2>To:<\/h2>\s*<p class=MsoNormal>&nbsp;<\/p>/, 
          `<h2>To:</h2>\n  <p class=MsoNormal>${data.clientInfo}</p>`);
      }
